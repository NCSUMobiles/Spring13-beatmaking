...
*/
	...
	public byte[] createDataBuffer(PriorityQueue<Sound> trackSoundQueue, Context ctxt){
		//myByteRate = 
		...
		double offset=0;
		//int startTime = 0;
		double currentTime = 0; // shows the amount of track (in millisecs) already processed.
		...
		while(tempPQ.size()>0){
			...
			if(offset>currentTime){
				...
				Byte silence = 0;
				int numberofsilencebytes = (int)Math.ceil(((float)(myByteRate)/1000.00) * differenceInMillisecs);  // myByteRate = 28 bytes /sec = 28 /1000 bytes per millisecs.
			...
			//TO DO implement the below function. Currently it returns a dummy file name 
	...
	public byte[] read(Context ctxt,String fileName){
		...
		try
		{
			...
			int resourceId = com.example.beatmakingapp.R.raw.class.getDeclaredField(fileName).getInt(new Integer(0)); 
	...
	public boolean save(Context ctxt,String fileName,byte[] myData)
	{
		try
		{
			...
			File projectDir = new File(sdcard.getAbsolutePath()+"/Beats/exported/");
			projectDir.mkdirs();
			...
			DataOutputStream outFile  = new DataOutputStream(new FileOutputStream(myFile));
